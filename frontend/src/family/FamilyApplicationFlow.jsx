...
export function FamilyApplicationFlow() {
  ...
  const [sponsorName, setSponsorName] = useState(profile?.fullNameEnglish || '')
  const [sponsorEmail, setSponsorEmail] = useState(profile?.email || '')
  const [sponsorPhone, setSponsorPhone] = useState(profile?.mobileNumber || '')
  ...
  useEffect(() => {
    if (!profile) return
    setSponsorName((value) => value || profile.fullNameEnglish || '')
    setSponsorEmail((value) => value || profile.email || '')
    setSponsorPhone((value) => value || profile.mobileNumber || '')
  }, [profile])
          ...
          <div class="family-section">
            ...
            <div class="family-field-row">
              <div class="family-field">
                <label>{t.sponsorNameLabel || 'Full name'} <UaePassBadge /></label>
                <input type="text" value={sponsorName} onInput={(e) => setSponsorName(e.target.value)} />
              </div>
              <div class="family-field">
                <label>{t.sponsorEmailLabel || 'Email'} <UaePassBadge /></label>
                <input type="email" value={sponsorEmail} onInput={(e) => setSponsorEmail(e.target.value)} />
              </div>
              <div class="family-field">
                <label>{t.sponsorPhoneLabel || 'Phone (optional)'} <UaePassBadge /></label>
            ...
            <div class="family-doc-list">
              {sponsorRequired.map((doc) => (
                <label key={doc} class="family-doc-checkbox">
                  <input type="checkbox" checked={!!sponsorDocs[doc] || !!matchDocument(doc)} onChange={() => toggleSponsorDoc(doc)} />
                  {doc}
                  {matchDocument(doc) && <UaePassBadge />}
                </label>
              ))}
            </div>
          </div>
